	...
	{ 
		...
		{ 
			...
			while(x!=0)
			{
				 ...
				 else { AI(&c,&d); qipan[d][c]=COM; x--; system("cls"); draw(); } /* 电脑下

子 */
				 c1=c; d1=d; /* 储存电脑上手棋型 */
				 if(win(c,d)){ /* 电脑赢 */printf("要悔棋吗？请输入y或n："); 
				 getchar(); 
				 scanf("%c",&ch);
				 while(ch!='y'&&ch!='n') { printf("输入错误，请重新输入：");
				 scanf("%c",&ch); 
			}
			 if(ch=='n') {printf("面对电脑，你居然会输，这不符合现实!!!\n"); 
			 return ; 
		}
	    else { x+=2; qipan[d][c]=SPA; qipan[b1][a1]=SPA;system("cls"); draw(); } /* 悔棋 */
	}
	 printf("电脑下在%d %d\n请输入：",c,d); 
	 scanf("%d%d",&a,&b); /* 玩家下子 */
	 if(a==15&&b==15)
...
[2],p[i][3],p[i][4],p[i][5],p[i][6],p[i][7],p[i][8],p[i][9],p[i][10],p[i][11],p[i][12],p[i][13],p[i]

[14],j);
	printf("  ├—┼—┼—┼—┼—┼—┼—┼—┼—┼—┼—┼—┼—┼—┼—┤\n"); } 
  printf("14│%s│%s│%s│%s│%s│%s│%s│%s│%s│%s│%s│%s│%s│%s│%s│14\n",p[14][0],p[14][1],p[14]

[2],p[14][3],p[14][4],p[14][5],p[14][6],p[14][7],p[14][8],p[14][9],p[14][10],p[14][11],p[14][12],p[14]
...
{40,400,3000,10000,6,10,600,10000,20,120,200,0,6,10,500,0,30,300,2500,5000,2,8,300,8000,26,160,0,0,4,20,300

,0};	 /* 数组a中储存己方和对方共32种棋型的值 己方0对方1 活0冲1空活2空冲3 子数0-3（0表示1个子，3表示4个子
...
） */	
	...
	{	
		...
		k2/=10; 
		X1=k1%10; 
		X2=k2%10;	 /* X Y Z分别表示 己方0对方1 活0冲1空活2空冲3 子数0-3（0表示1个子，3表示4个
...
子） */	
		...
		if(((K1>-1&&K1<4)&&((K2>-1&&K2<4)||(K2>9&&K2<14)))||((K1>99&&K1<104)&&((K2>99&&K2<104)||
...
if(((K1>-1&&K1<4)&&((K2>99&&K2<104)||(K2>109&&K2<114)))||((K1>9&&K1<14)&&((K2>99&&K2<104)||
...
{	
int k,m=0; /* 棋型号注解: 己活000-003 己冲010-013 对活100-103 对冲110-113 己空活020-023 己空冲030-033 对空

活120-123 对空冲130-133 空-1 边界冲-2 边界空冲-3*/
yiwei(n,&p,&q);	
